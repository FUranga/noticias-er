...
- `docs/fundacion-hallazgos-institucionales.md` — captura corriente de hallazgos de vacíos institucionales/de contrapesos para FUNDER (formato liviano, sin pulir), a medida que aparecen investigando otra cosa. Se traslada a `fundacion-agenda-institucional.md` cuando toma forma de diagnóstico o propuesta. Deliberadamente fuera de la cablera, para no mezclar la agenda de FUNDER con el pipeline editorial del medio.
...
- `data/backlog.json` — la cablera: comunicados candidatos con estado editorial (`pendiente`/`a_publicar`/`referido`/`a_investigar`/`descartado`). `referido` es para lo noticiable pero ya bien cubierto por otro medio: se atribuye y linkea (redes/newsletter/portada), no se reescribe — es la aplicación práctica de "agencia, no repetición" (`docs/vision-y-etapas.md`) y de la sección "Medios y empresas como fuente" de `docs/estilo-editorial.md`. `a_investigar` es para lo que puede ser un hueco/ángulo propio pero necesita más reporteo antes de decidir. No hay estado `procesado`: un ítem ya subido a WordPress se marca con `wp_edit_url`/`procesado_el` sin cambiar de estado. Se edita desde `admin/index.html`, no a mano.
- `data/archivo/` — items que salieron de `data/backlog.json` para que no crezca para siempre (`pipeline/archivar_cablera.py`, corrido por `.github/workflows/archivar_descartados.yml` semanal y `archivar_publicados.yml` mensual): `backlog-descartados.json` (estado `descartado`/`referido`) y `backlog-publicados.json` (`a_publicar` con `wp_edit_url` ya cargado). No aparecen en `admin/index.html`, pero siguen buscables ahí si hace falta revisar un antecedente.
