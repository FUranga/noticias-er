...
2. **Para cada ítem**, en orden:
   ...
   3. **Conseguir imagen** (best-effort, nunca bloqueante):
      ...
      - Si no, y el ítem tiene `link`: traer la página (WebFetch) y buscar la imagen principal — meta tag `og:image` primero, si no hay, la primera imagen grande del cuerpo del artículo. Si no se encuentra nada con confianza razonable, seguir sin imagen — no es un bloqueante, no inventar una URL de imagen.
