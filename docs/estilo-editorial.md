...
- **Profundidad como estándar por defecto, para cualquier nota — no una excepción reservada a casos grandes** (Francisco, 2026-09-13: "así hay que proceder siempre para cualquier historia", corrigiendo una primera versión de esta regla que la limitaba a notas reconstruidas de varios medios). Cada tema que aparece en el cuerpo — una cifra, un organismo, una decisión previa, un conflicto — necesita la explicación mínima para que un lector nuevo lo entienda: qué es, por qué importa, qué lo vuelve conflictivo, a quién impacta y cómo. Si al escribir una oración no se puede explicar en una frase más por qué ese dato importa o de dónde sale, es señal de que falta investigar ese punto, no de que alcanza con mencionarlo y seguir. Esto vale para una nota de un comunicado único tanto como para una reconstruida de varios medios — ver la sección de más abajo para el caso en que esto es más fácil de pasar por alto.
...
- **Preferí la interpretación/el enfoque del hecho antes que apilar cifras — pero con cuidado de qué "Bloomberg" estamos imitando.** Bloomberg tiene dos registros bien distintos: el de mercados/finanzas (ahí el número va adelante porque el dato es literalmente el producto — "Fed Raises Rates to Highest Since 2007") y el de política/negocios en general, donde los títulos son interpretativos ("JPMorgan's Blessing Says Dealmaking Could Hit Record", "Midterm Primaries Are Showing a Voter Enthusiasm Gap") — afirmación o enfoque, no una cifra suelta. Este medio no es un wire de mercados: cubre política y economía institucional de una provincia, así que el registro que corresponde imitar es el segundo. Por eso, **el título por defecto lleva sujeto + acción + qué pasó (interpretación/enfoque), no una cifra.**
  - Un número exacto en el título es la **excepción**, reservada para cuando ese dato *es* la noticia. **Pero incluso ahí, el título tiene que leerse como una nota de política/economía institucional, no como un ticker de mercado** — "Entre Ríos exportó 14 millones de dólares en servicios basados en conocimiento en el primer trimestre" es formalmente correcto (una sola cifra) pero suena a wire financiero por la estructura completa de la oración, no solo por el número. Preferí anclar la cifra a la interpretación del hecho en vez de encabezar con el dato puro — por ejemplo algo en la línea de "Entre Ríos aceleró sus exportaciones de servicios basados en conocimiento en el primer trimestre" con la cifra exacta bajando a la bajada o primer párrafo. **Caso real (2026-09, nota del MiradorTec/SBC): el editor marcó el título y la bajada como "muy numerosos" pese a cumplir la letra de la regla — la lección es que "una sola cifra" es necesario pero no suficiente; el tono general de la oración también tiene que sonar a interpretación, no a reporte de cifras.**
  - **La bajada es más propensa a este error que el título.** Apilar dos datos de contexto en la bajada (ej. "sube 10% interanual" + "es récord histórico nacional") sin ligarlos con una interpretación hace que se lea como una lista de estadísticas, aunque cada oración individualmente tenga una sola cifra. La bajada tiene que sumar UN dato o matiz que el título no dio, no una segunda ronda de cifras de contexto — el resto de las cifras (nacionales, históricas, comparativas) van al cuerpo.
  - Nunca apilar más de un número en el título ni en la bajada; si hay varias cifras relevantes, van en el cuerpo.
...
- **Moneda siempre en letras**: "1.200 millones de pesos", nunca "$1.200 millones" ni "u$s". **Excepción confirmada (Francisco, 2026-09-07)**: precios unitarios o tarifarios (ej. $/kWh, precio por unidad, cualquier cifra con centavos que se está comparando entre sí, tabla de precios) van siempre con el símbolo "$" (o "USD"/"u$s" si es en dólares), nunca escritos en letras — al revés de la regla general. La regla de "en letras" es para montos grandes y redondos (inversión, presupuesto, gasto total), no para precios unitarios de consumo, donde el símbolo es más legible y es la convención esperada (caso real: nota sobre el cuadro tarifario eléctrico de ENERSA, comparando precios de $/kWh entre meses).
...
- **Variar el verbo de atribución** dentro de la misma nota — no repetir siempre "dijo" o "según". Banco de verbos: dijo, aseguró, afirmó, sostuvo, explicó, señaló, destacó, subrayó, indicó, precisó, detalló, advirtió, consideró, argumentó, planteó, admitió, informó. Reservar los verbos con carga interpretativa (advirtió, admitió) para cuando el tono de la cita lo amerita, no como sinónimo neutro de "dijo".
- **Nunca usar "consignó"** como verbo de atribución (pedido explícito de Francisco, 2026-09-15) — usar "informó" en su lugar.
...
**No sobre-atribuir un dato que surge directo de comparar dos documentos oficiales** (Francisco, 2026-09-07, caso real: nota sobre el cuadro tarifario eléctrico). Comparar dos cifras que publica el mismo organismo en dos resoluciones sucesivas (ej. el precio de un tramo tarifario en la resolución de septiembre vs. la de octubre) y sacar el porcentaje de diferencia no es "un cálculo propio" que necesite el envoltorio "según un cálculo de Agencia Entrerriana sobre..." — es lo que dicen las resoluciones, y hay que escribirlo como un hecho directo ("subió 31,5%"), citando la fuente donde corresponda en el cuerpo (no en el lede, y sin la muletilla de "cálculo propio"). Reservar el envoltorio de "cálculo propio"/"verificación independiente" para cuando el dato realmente requiere una operación editorial no trivial (cruzar series de distintas fuentes, ajustar por inflación, etc.), no para leer dos números publicados por el mismo organismo. Además: **no acumular atribución en el lede** — resulta forzado y ilegible; si hace falta citar la fuente, hacerlo en un párrafo posterior del cuerpo.
...
**Esta regla es para un comunicado único.** Cuando la nota se reconstruye a partir de varios medios sobre un conflicto en curso, ver la sección "Notas reconstruidas a partir de varios medios" más abajo — ahí la nota corta es el error, no el default.
...
- **Si existe una serie de datos propia (`data/series/`, `docs/fuentes-datos.md`) sobre el mismo tema que cubrió otro medio, cruzarla antes de escribir — es el aporte propio más directo que hay.** Caso real (2026-09-13): varios medios citaron un informe de CEPA/SRT que estimaba en 13.500 los empleos perdidos en Entre Ríos desde 2023. La serie propia del proyecto hermano `mapa-empleo` (OEDE/SIPA, ver `docs/fuentes-datos.md`) mide específicamente empleo privado y da una caída menor en términos absolutos (7.116 puestos) — pero al calcular el porcentaje sobre el universo total que sí mide la SRT, el guarismo relativo es casi idéntico (~5% en las dos). **La solución no es elegir una cifra y descartar la otra, ni presentarlas como si midieran lo mismo**: se explica que cada fuente mide un universo distinto (todo el empleo registrado vs. solo el privado) y se muestra que, pese a eso, ambas apuntan en la misma dirección — eso es más fuerte editorialmente que repetir la cifra de un solo informe, y es exactamente el tipo de valor agregado que pide la regla de "usar la cobertura ajena como disparador, no como producto final" de más arriba.

## Notas reconstruidas a partir de varios medios (conflicto o proceso en curso)

Caso particular donde la regla general de profundidad de arriba es más fácil de pasar por alto: cuando la cablera trae un hecho ya cubierto por varios medios locales sobre un conflicto con varias voces (una interna partidaria, una disputa gremial, una pelea institucional) y el editor decide igual escribir una nota propia. Caso real (2026-09-13): una nota sobre la expulsión de dos senadoras del PJ entrerriano, reconstruida a partir de ocho artículos de seis medios distintos. La primera versión falló varios de estos puntos a la vez — quedó "subexplicada", con datos sin verificar y una cita sin atribuir — y el editor la mandó a reescribir entera.

- **La regla de "Extensión: corta" de arriba es para un comunicado único.** Acá casi nunca alcanza: una nota que reconstruye un conflicto con varias voces necesita el espacio para explicar el contexto técnico/institucional de fondo (qué hace la medida en disputa, por qué es controversial, cómo llegó a este punto) — 3 a 6 párrafos casi seguro la deja "subexplicada". Preferir una nota más larga y completa a una corta que obliga al lector a saber de antemano de qué se trata.
- **No armar el cuerpo pegando fragmentos de los artículos fuente sin investigar el contexto que ninguno de ellos explica del todo.** Los medios locales suelen asumir que el lector ya sigue el tema — no explican, por ejemplo, cómo está compuesto el cuerpo legislativo, o qué cambia técnicamente una reforma. Si un dato hace falta para que la nota se entienda sola y no está en ninguna de las fuentes recopiladas, buscarlo aparte (fuente primaria del organismo, otra cobertura) antes de escribir — no dejarlo como una frase vaga sin explicar (ej. "el oficialismo ya contaba con mayor margen" sin decir de cuánto, sobre cuántas bancas).
- **Cada cita textual necesita su atribución real y verificable, no un resumen sin marcar.** Si una frase entre comillas viene de un medio que a su vez la sacó de una radio/entrevista puntual, attribuir las dos capas: "según publicó [medio], en diálogo con [programa/medio original]". Nunca se pone una cita entre comillas sin poder decir de dónde salió literalmente — es la falla más grave que puede tener una nota de este tipo.
- **Buscar la voz de contraste de los dos lados del conflicto, no solo de uno.** Si el hecho principal ya tiene la voz de quien sancionó/decidió, buscar activamente si alguien de adentro del mismo espacio disintió (una interna no es monolítica) y la respuesta del sancionado, no solo asumir que "no hubo otra voz" porque las fuentes recopiladas no la traían de entrada.
- **La foto tiene que ser del sujeto real de la nota, no un genérico de la ciudad/provincia.** Si la nota es sobre personas identificables (funcionarios, legisladores), Nivel 1/2 de `docs/politica-imagenes.md` casi siempre da una foto oficial real de esas personas (sitio institucional del cuerpo al que pertenecen, ej. la ficha de un legislador en la web de la Legislatura) — buscarla antes de bajar a una foto de lugar genérica.
