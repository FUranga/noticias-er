...
2. **Backlog de comunicados entrantes — "la cablera"** (`data/backlog.json`): un ítem por comunicado/novedad candidata, con estado `pendiente` / `a_publicar` / `descartado` / `procesado`. Al principio se carga a mano (revisando cada portal/RSS, o con ayuda de la skill `mapear-fuentes`); más adelante, automatizable por fuente (RSS reader, scraper liviano) a medida que `fuentes.md` tenga feeds confirmados — no hace falta un scraper sofisticado desde el día uno.
