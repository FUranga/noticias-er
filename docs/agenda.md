...
3. **Manual, sin ítem de origen (2026-09-07)**: la macro-pestaña **"Agenda"** del panel (junto a "Cablera" y "Boletín Oficial") lista todo `data/agenda.json`, con sus propios sub-estados (`pendiente` / `cubierto` / `vencido_sin_seguimiento`, ordenado por fecha) y un botón "+ Agregar evento" para cargar algo que no vino de ningún comunicado puntual — ej. una fecha que Francisco supo por otro medio. En ese caso `origen_id` queda `null` y `fuente` se completa a mano. Marcar "cubierto" pide opcionalmente el link/referencia de la nota que lo cubrió y lo agrega a `detalle`.
