...
**Panel admin (2026-09-06/07, estados rediseñados 2026-09-07, ver `admin/index.html`)**: el Boletín tiene su propia **macro-pestaña** ("Boletín Oficial", junto a "Cablera") que cambia toda la vista, no un filtro adicional — un ítem del Boletín no aparece mezclado en "Cablera" (mismo motivo que el filtro mecánico: evitar volumen). Adentro tiene **4 estados propios y reales**, no una relabel del genérico (`ESTADOS_BOLETIN` en `admin/index.html`, distinto de `ESTADOS_GENERAL` de la Cablera): Pendientes / A publicar / A investigar / Descartadas, ordenadas por `prioridad`. Los dos estados positivos son decisiones editoriales distintas, no un matiz de la misma:
