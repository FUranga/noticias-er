...
- **Sección "Boletín Oficial" propia en el panel admin** y **skill de investigación** (`investigar-boletin`, con las dos salidas: nota lista vs. entrada en `docs/temas-a-seguir.md`) — diseño acordado con Francisco (2026-09-06), todavía no construido. Hoy los ítems llegan a `data/backlog.json` con el campo `prioridad` ya calculado, listos para que el panel los agrupe cuando se construya esa vista.
...
La edición de referencia (Nº 28.410) quedó publicada (PDF con `Last-Modified`) a las 16:24 GMT (~13:24 hora Argentina) del mismo día de la edición. Un solo dato — el cronograma en `.github/workflows/monitorear_boletin_er.yml` corre una vez por día a media tarde (hora Argentina) como primera aproximación; ajustar si se observa un patrón distinto con más ediciones.
